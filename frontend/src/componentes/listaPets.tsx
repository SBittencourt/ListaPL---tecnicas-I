...
const ListaPet: React.FC<Props> = ({ tema, seletorView }) => {
    ...
    const [petSelecionadoIndex, setPetSelecionadoIndex] = useState<number | null>(null);

    const handleClick = (clienteIndex: number, petIndex: number) => {
        if (petSelecionadoIndex === null || petSelecionadoIndex !== petIndex) {
            setClienteSelecionadoIndex(clienteIndex);
            setPetSelecionadoIndex(petIndex);
        } else {
            setPetSelecionadoIndex(null);
        }
    };

    const handleExcluirPet = (clienteIndex: number, petIndex: number) => {
        console.log(`Excluir pet ${petIndex} do cliente ${clienteIndex}`);
    };

    const handleAtualizarPet = (clienteIndex: number, petIndex: number) => {
                    ...
                    cliente.pets.map((pet, petIndex) => (
                        <div key={`${clienteIndex}-${petIndex}`}>
                            <a
                                ...
                                onClick={() => handleClick(clienteIndex, petIndex)}
                                ...
                                {clienteSelecionadoIndex === clienteIndex && petSelecionadoIndex === petIndex && (
                                    <div className="card mt-3">
                                        <div className="card-body">
                                            ...
                                            <div className="mt-3">
                                                <button className="btn btn-danger btn-sm ml-2" onClick={() => handleExcluirPet(clienteIndex, petIndex)}>Excluir</button>
                                                <button className="btn btn-primary btn-sm ml-2" onClick={() => handleAtualizarPet(clienteIndex, petIndex)}>Atualizar</button>
                                            </div>
                                        </div>
                                    </div>
                                )}
